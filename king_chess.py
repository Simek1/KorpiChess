...
class ch_board(object):
	...
	def pos_areas(self):
		...
		for i in range(0, 8):
			...
			x += self.area
		return(table)
	def add_positions(self, white_pawns, black_pawns, turn, pawn, w_destroyed, b_destroyed):
		positions=[]
		en_king=[]
		king_pos=[-1,-1]
		if turn=="white":
		...
		while playing:
			...
			while adding:  # Dodawanie figury na szachownice
				if figure==0:
					...
					possible_pos=board.add_positions(white_pawns, black_pawns, turn, temp, w_destroyed, b_destroyed)
